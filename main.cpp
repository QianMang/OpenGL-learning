...
int main() {
	...
	reallyShinyMaterial = Material(8.0f, 312);
	...
	//avent = Model();
	//avent.LoadModel("Models/Avent.obj");
	...
	glm::mat4 projection = glm::perspective(45.0f, mainWindow.getBufferWidth()/ mainWindow.getBufferHeight(), 0.1f,50.0f );

	

	//Loop until window closed
	while (!mainWindow.getShouldCloser()) {
		...
		glUniformMatrix4fv(uniformView, 1, GL_FALSE, glm::value_ptr(camera.caculateViewMatrix()));
		...
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		reallyShinyMaterial.UseMaterial(uniformSpecularIntensity, uniformShininess);
		xwing.RenderModel();
